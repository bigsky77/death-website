...
function getRscError(fileName, err, module, compilation, compiler) {
    ...
    const error = new _simpleWebpackError.SimpleWebpackError(fileName, formattedError[0] + formattedError[1] + moduleTrace.map((m)=>m.resource ? "  " + (0, _path).relative(compiler.context, m.resource) : "").filter(Boolean).join("\n"));
