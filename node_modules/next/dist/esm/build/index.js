...
import { lockfilePatchPromise, teardownTraceSubscriber, teardownCrashReporter } from "./swc";
...
export default async function build(dir, conf = null, reactProductionProfiling = false, debugOutput = false, runLint = true) {
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            try {
                const [[verifyResult, typeCheckEnd]] = await Promise.all([
                    ...
                    shouldLint && nextBuildSpan.traceChild("verify-and-lint").traceAsyncFn(async ()=>{
                        var ref;
                        await verifyAndLint(dir, eslintCacheDir, (ref = config.eslint) == null ? void 0 : ref.dirs, config.experimental.cpus, config.experimental.workerThreads, telemetry, hasAppDir);
                    }), 
                ]);
                ...
                if (isError(err1) && err1.message === "Call retries were exceeded") {
                    process.exit(1);
                }
                throw err1;
            }
            ...
            const hasPages404 = (ref7 = mappedPages["/404"]) == null ? void 0 : ref7.startsWith(PAGES_DIR_ALIAS);
            ...
            const appPathRoutes = {};
            if (appDir) {
                appPathsManifest = JSON.parse(await promises.readFile(path.join(distDir, serverDir, APP_PATHS_MANIFEST), "utf8"));
                        ...
                        if (!isReservedPage(page)) {
                            try {
                                ...
                                if (pageRuntime === SERVER_RUNTIME.edge) {
                                    const manifest = require(join(distDir, serverDir, MIDDLEWARE_MANIFEST));
            ...
            const middlewareManifest = JSON.parse(await promises.readFile(path.join(distDir, serverDir, MIDDLEWARE_MANIFEST), "utf8"));
            ...
            if (combinedPages.length > 0 || useStatic404 || useDefaultStatic500 || hasAppDir) {
                ...
                await staticGenerationSpan.traceAsyncFn(async ()=>{
                    ...
                    for (const page2 of staticPages){
                        const serverBundle = getPagePath(page2, distDir, isLikeServerless);
                    ...
                    const moveExportedPage = async (originPage, page, file, isSsg, ext, additionalSsgFile = false)=>{
                        return staticGenerationSpan.traceChild("move-exported-page").traceAsyncFn(async ()=>{
                            ...
                            const relativeDest = path.relative(path.join(distDir, serverDir), path.join(path.join(pagePath, // strip leading / and then recurse number of nested dirs
                            ...
                            if (i18n) {
                                ...
                                for (const locale of i18n.locales){
                                    ...
                                    const updatedDest = path.join(distDir, serverDir, updatedRelativeDest);
                                    if (!isSsg) {
            ...
            images.remotePatterns = ((config == null ? void 0 : (ref1 = config.images) == null ? void 0 : ref1.remotePatterns) || []).map((p)=>({
                    ...
                    port: p.port,
                    pathname: makeRe((_pathname = p.pathname) != null ? _pathname : "**").source
                }));
            ...
            await nextBuildSpan.traceChild("print-tree-view").traceAsyncFn(()=>printTreeView(pageKeys, allPageInfos, isLikeServerless, {
                    distPath: distDir,
                    buildId: buildId,
                    pagesDir,
                    useStatic404,
