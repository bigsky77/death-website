...
export async function printTreeView(lists, pageInfos, serverless, { distPath , buildId , pagesDir , pageExtensions , buildManifest , appBuildManifest , middlewareManifest , useStatic404 , gzipSize =true  }) {
    var ref6;
    ...
    const printFileTree = async ({ list , routerType  })=>{
        ...
        filterAndSortList(list).forEach((item, i, arr)=>{
            ...
            const symbol = routerType === "app" || item === "/_app" || item === "/_app.server" ? " " : (pageInfo == null ? void 0 : pageInfo.static) ? "\u25CB" : (pageInfo == null ? void 0 : pageInfo.isSsg) ? "\u25CF" : (pageInfo == null ? void 0 : pageInfo.runtime) === SERVER_RUNTIME.edge ? "\u2107" : "\u03BB";
...
export const collectGenerateParams = (segment, parentSegments = [], generateParams = [])=>{
...
export async function isPageStatic({ page , distDir , serverless , configFileName , runtimeEnvConfig , httpAgentOptions , enableUndici , locales , defaultLocale , parentId , pageRuntime , edgeInfo , pageType , hasServerComponents , originalAppPath  }) {
...
export async function hasCustomGetInitialProps(page, distDir, isLikeServerless, runtimeEnvConfig, checkingApp) {
    require("../shared/lib/runtime-config").setConfig(runtimeEnvConfig);
...
export async function getNamedExports(page, distDir, isLikeServerless, runtimeEnvConfig) {
    require("../shared/lib/runtime-config").setConfig(runtimeEnvConfig);
    const components = await loadComponents({
        distDir,
        pathname: page,
...
export async function copyTracedFiles(dir, distDir, pageKeys, tracingRoot, serverConfig, middlewareManifest) {
    ...
    await fs.writeFile(serverOutputPath, `
