...
const rscSharedRegex = /(node_modules[\\/]react\/|[\\/]shared[\\/]lib[\\/](head-manager-context|router-context|server-inserted-html)\.js|node_modules[\\/]styled-jsx[\\/])/;
...
function isResourceInPackages(resource, packageNames) {
    return packageNames == null ? void 0 : packageNames.some((p)=>new RegExp("[/\\\\]node_modules[/\\\\]" + p + "[/\\\\]").test(resource));
}
...
export default async function getBaseWebpackConfig(dir, { buildId , config , compilerType , dev =false , entrypoints , hasReactRoot , isDevFallback =false , pagesDir , reactProductionProfiling =false , rewrites , runWebpackSpan , target =COMPILER_NAMES.server , appDir , middlewareMatchers  }) {
    var ref47, ref4, ref5, ref6, ref7, ref8, ref9, ref10, ref11, ref12, // allows add JsConfigPathsPlugin to allow hot-reloading
    ...
    if (isClient) {
        if (!hasReactRoot) {
            ...
            if (hasAppDir) {
                throw new Error("`experimental.appDir` requires React 18 to be installed.");
            }
        }
    }
    ...
    const getSwcLoader = (extraOptions)=>{
        ...
        return {
            ...
            options: {
                ...
                swcCacheDir: path.join(dir, (ref46 = config == null ? void 0 : config.distDir) != null ? ref46 : ".next", "cache", "swc"),
                ...extraOptions
            }
        };
    };
    ...
    const outputPath = isNodeServer || isEdgeServer ? path.join(distDir, isLikeServerless ? SERVERLESS_DIRECTORY : SERVER_DIRECTORY) : distDir;
    const clientEntries = isClient ? {
        ...
        ...dev ? {
            ...
            [CLIENT_STATIC_FILES_RUNTIME_AMP]: `./` + relativePath(dir, pathJoin(NEXT_PROJECT_ROOT_DIST_CLIENT, "dev", "amp-dev")).replace(/\\/g, "/")
        } : {},
    ...
    async function handleExternals(context, request, dependencyType, layer, getResolve) {
        ...
        if (layer === WEBPACK_LAYERS.server) {
            if (request === "react" || request === "react/jsx-runtime" || request === "next/dist/compiled/react-server-dom-webpack/writer.browser.server") {
                return;
            }
        }
        ...
        if (/node_modules[/\\].*\.[mc]?js$/.test(res)) {
            if (layer === WEBPACK_LAYERS.server) {
                ...
                if (isResourceInPackages(res, config.experimental.serverComponentsExternalPackages)) {
                    return `${externalType} ${request}`;
                }
                return;
            }
    ...
    const fontLoaderTargets = config.experimental.fontLoaders && Object.keys(config.experimental.fontLoaders).map((fontLoader)=>{
    ...
    let webpackConfig = {
        ...
        plugins: [
            ...
            target === "serverless" && (isNodeServer || isEdgeServer) && new ServerlessPlugin(),
            ...
            isClient && config.experimental.fontLoaders && new FontLoaderManifestPlugin(),
    ...
    const cache = {
        ...
        version: `${"12.3.2-canary.21"}|${configVars}`,
    ...
    var ref39;
    const hasUserCssConfig = (ref39 = (ref24 = webpackConfig.module) == null ? void 0 : (ref25 = ref24.rules) == null ? void 0 : ref25.some((rule)=>canMatchCss(rule.test) || canMatchCss(rule.include))) != null ? ref39 : false;
    if (hasUserCssConfig) {
