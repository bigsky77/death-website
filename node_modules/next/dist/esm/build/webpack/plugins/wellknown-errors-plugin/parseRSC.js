...
export function formatRSCErrorMessage(message) {
...
export function getRscError(fileName, err, module, compilation, compiler) {
    ...
    const error = new SimpleWebpackError(fileName, formattedError[0] + formattedError[1] + moduleTrace.map((m)=>m.resource ? "  " + relative(compiler.context, m.resource) : "").filter(Boolean).join("\n"));
