...
export async function runTypeCheck(ts, baseDir, tsConfigPath, cacheDir) {
    ...
    var ref;
    const firstError = (ref = allDiagnostics.find((d)=>d.category === DiagnosticCategory.Error && Boolean(d.file))) != null ? ref : allDiagnostics.find((d)=>d.category === DiagnosticCategory.Error);
    if (firstError) {
        throw new CompileError(await getFormattedDiagnostic(ts, baseDir, firstError));
    }
    const warnings = await Promise.all(allDiagnostics.filter((d)=>d.category === DiagnosticCategory.Warning).map((d)=>getFormattedDiagnostic(ts, baseDir, d)));
    return {
        hasWarnings: true,
        warnings,
