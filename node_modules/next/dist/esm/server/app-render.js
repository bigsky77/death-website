 ...
 */ function createErrorHandler(/**
   ...
   */ _source, capturedErrors) {
    return (err)=>{
        ...
        err.digest !== DYNAMIC_ERROR_CODE && err.digest !== NOT_FOUND_ERROR_CODE && !((ref = err.digest) == null ? void 0 : ref.startsWith(REDIRECT_ERROR_CODE))) {
 ...
 */ function useFlightResponse(writable, req, serverComponentManifest, rscChunks, flightResponseRef, nonce) {
    ...
    function read() {
        forwardReader.read().then(({ done , value  })=>{
            ...
            if (!bootstrapped) {
                bootstrapped = true;
                writer.write(encodeText(`${startScriptTag}(self.__next_s=self.__next_s||[]).push(${htmlEscapeJsonString(JSON.stringify([
                    0
                ]))})</script>`));
            }
            if (done) {
                ...
                const scripts = `${startScriptTag}(self.__next_s=self.__next_s||[]).push(${htmlEscapeJsonString(JSON.stringify([
                    1,
                    responsePartial
                ]))})</script>`;
...
function getRootLayoutPath([segment, parallelRoutes, { layout  }], rootLayoutPath = "") {
...
function findRootLayoutInFlightRouterState([segment, parallelRoutes], rootLayoutSegments, segments = "") {
    segments += `${segment}/`;
...
function isNavigatingToNewRootLayout(loaderTree, flightRouterState) {
    const newRootLayout = getRootLayoutPath(loaderTree);
    // should always have a root layout
    if (newRootLayout) {
        const hasSameRootLayout = findRootLayoutInFlightRouterState(flightRouterState, newRootLayout);
        return !hasSameRootLayout;
    }
    return false;
}
export async function renderToHTMLOrFlight(req, res, pathname, query, renderOpts, isStaticGeneration = false) {
    ...
    const { buildManifest , subresourceIntegrityManifest , serverComponentManifest , serverCSSManifest ={} , supportsDynamicHTML , ComponentMod , dev ,  } = renderOpts;
     ...
     */ const providedFlightRouterState = isFlight ? req.headers.__next_router_state_tree__ ? JSON.parse(req.headers.__next_router_state_tree__) : {} : undefined;
        /**
     * The tree created in next-app-loader that holds component segments and modules
     */ const loaderTree = ComponentMod.tree;
        // If navigating to a new root layout we need to do a full page navigation.
        if (isFlight && Array.isArray(providedFlightRouterState) && isNavigatingToNewRootLayout(loaderTree, providedFlightRouterState)) {
            stripInternalQueries(query);
            const search = stringifyQuery(query);
            // Empty so that the client-side router will do a full page navigation.
            const flightData = req.url + (search ? `?${search}` : "");
            return new FlightRenderResult(ComponentMod.renderToReadableStream(flightData, serverComponentManifest, {
     ...
     */ const createComponentTree = async ({ createSegmentPath , loaderTree: [segment, parallelRoutes, { layoutOrPagePath , layout , template , error , loading , page , "not-found": notFound ,  }, ] , parentParams , firstItem , rootLayoutIncluded  })=>{
       ...
       */ const currentParams = // Handle null case where dynamic param is optional
            ...
            return {
                Component: ()=>{
                    let props = {};
                    return /*#__PURE__*/ React.createElement(React.Fragment, null, stylesheets ? stylesheets.map((href)=>/*#__PURE__*/ React.createElement("link", {
                            rel: "stylesheet",
         ...
         */ const renderComponentsOnThisLevel = // No further router state available
                ...
                if (!parentRendered && renderComponentsOnThisLevel) {
                    return [
                        ...
                        isPrefetch && !Boolean(loaderTreeToFilter[2].loading) ? null : /*#__PURE__*/ React.createElement((await createComponentTree(// This ensures flightRouterPath is valid and filters down the tree
                        {
                            createSegmentPath: (child)=>{
     ...
     */ const ServerComponentsRenderer = createServerComponentRenderer(()=>{
            ...
            return /*#__PURE__*/ React.createElement(AppRouter, {
                hotReloader: HotReloader && /*#__PURE__*/ React.createElement(HotReloader, {
        ...
        const bodyResult = async ()=>{
            ...
            const getServerInsertedHTML = ()=>{
                const flushed = renderToString(/*#__PURE__*/ React.createElement(React.Fragment, null, Array.from(serverInsertedHTMLCallbacks).map((callback)=>callback())));
                return flushed;
            };
