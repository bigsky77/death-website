...
export function calculateOverrideValues(font, fontMetrics) {
...
export function calculateSizeAdjustValues(font, fontMetrics) {
    const fontKey = font.trim();
    let { category , ascent , descent , lineGap , unitsPerEm , xAvgCharWidth  } = fontMetrics[fontKey];
    const fallbackFont = category === "serif" ? DEFAULT_SERIF_FONT : DEFAULT_SANS_SERIF_FONT;
...
function calculateOverrideCSS(font, fontMetrics) {
    ...
    const { ascent , descent , lineGap , fallbackFont  } = calculateOverrideValues(font, fontMetrics);
    return `
    @font-face {
...
function calculateSizeAdjustCSS(font, fontMetrics) {
    const fontName = font.trim();
    const { ascent , descent , lineGap , fallbackFont , sizeAdjust  } = calculateSizeAdjustValues(font, fontMetrics);
    return `
    @font-face {
      font-family: "${fontName} Fallback";
      ascent-override: ${ascent}%;
