...
export async function loadComponents({ distDir , pathname , serverless , hasServerComponents , isAppPath  }) {
    if (serverless) {
    ...
    if (!isAppPath) {
        [DocumentMod, AppMod] = await Promise.all([
            Promise.resolve().then(()=>requirePage("/_document", distDir, serverless, false)),
            Promise.resolve().then(()=>requirePage("/_app", distDir, serverless, false)), 
        ]);
    }
    const ComponentMod = await Promise.resolve().then(()=>requirePage(pathname, distDir, serverless, isAppPath));
