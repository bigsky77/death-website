...
import { PAGES_MANIFEST, BUILD_ID_FILE, MIDDLEWARE_MANIFEST, CLIENT_STATIC_FILES_PATH, CLIENT_STATIC_FILES_RUNTIME, PRERENDER_MANIFEST, ROUTES_MANIFEST, FLIGHT_MANIFEST, CLIENT_PUBLIC_FILES_PATH, APP_PATHS_MANIFEST, FLIGHT_SERVER_CSS_MANIFEST, SERVERLESS_DIRECTORY, SERVER_DIRECTORY, FONT_LOADER_MANIFEST } from "../shared/lib/constants";
...
import BaseServer, { prepareServerlessUrl, NoFallbackError } from "./base-server";
...
export default class NextNodeServer extends BaseServer {
    ...
    compression = this.nextConfig.compress && this.nextConfig.target === "server" ? compression() : undefined;
    ...
    getHasAppDir() {
        ...
        return fs.existsSync(appDirectory) && fs.statSync(appDirectory).isDirectory();
    }
    ...
    getPagePath(pathname, locales) {
        return getPagePath(pathname, this.distDir, this._isLikeServerless, this.renderOpts.dev, locales, this.hasAppDir);
    }
   ...
   */ getEdgeFunctionInfo(params) {
        ...
        var _env, _wasm, _assets;
        return {
            name: pageInfo.name,
            paths: pageInfo.files.map((file)=>join(this.distDir, file)),
            env: (_env = pageInfo.env) != null ? _env : [],
   ...
   */ async runMiddleware(params) {
        ...
        const url = `${getRequestMeta(params.request, "_protocol")}://${this.hostname}:${this.port}${locale ? `/${locale}` : ""}${params.parsed.pathname}${query ? `?${query}` : ""}`;
    ...
    async runEdgeFunction(params) {
        ...
        if (result.response.body) {
            ...
            bodyStreamToNodeStream(result.response.body).pipe(params.res.originalResponse);
        } else {
