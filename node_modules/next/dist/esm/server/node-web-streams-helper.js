    ...
    return new TransformStream({
        ...
        flush (controller) {
            ...
            if (missingTags.length > 0) {
                controller.error("Missing required root layout tags: " + missingTags.join(", "));
            }
        }
    });
}
export async function continueFromInitialStream(renderStream, { dev , suffix , dataStream , generateStaticHTML , getServerInsertedHTML , serverInsertedHTMLToHead , polyfills  }) {
    ...
    const transforms = [
        ...
        createHeadInjectionTransformStream(async ()=>{
            ...
            const polyfillScripts = polyfills ? polyfills.map(({ src , integrity  })=>`<script src="${src}" nomodule=""${integrity ? ` integrity="${integrity}"` : ""}></script>`).join("") : "";
